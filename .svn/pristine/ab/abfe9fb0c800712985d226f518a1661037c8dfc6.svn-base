...
import com.ukang.baiyu.view.photo.adapter.AlbumGridViewAdapter;
import com.ukang.baiyu.view.photo.util.AlbumHelper;
import com.ukang.baiyu.view.photo.util.Bimp;
import com.ukang.baiyu.view.photo.util.ImageBucket;
...
public class AlbumActivity extends Activity{
	...
	private int netCount = 0;//网络图片总计
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(Res.getLayoutID("plugin_camera_album"));
		PublicWay.activityList.add(this);
		mContext = this;
		netCount = getIntent().getIntExtra("NET_IMG_COUNT", 0);
...
//			intent.setClass(mContext, AddMedChartActivity.class);
//			startActivity(intent);
//			finish();
			setResult(RESULT_OK);
			finish();
...
//			startActivity(intent);
	...
	private void init() {
		...
		back = (ImageButton) findViewById(Res.getWidgetID("back"));
...
//		preview.setOnClickListener(new PreviewListener());
		...
		okButton.setText(Res.getString("finish")+"(" + (Bimp.tempSelectBitmap.size() + netCount)
				+ "/"+PublicWay.num+")");
	}
		...
		gridImageAdapter
				.setOnItemClickListener(new AlbumGridViewAdapter.OnItemClickListener() {
					...
					public void onItemClick(final ToggleButton toggleButton,
						...
						if ((Bimp.tempSelectBitmap.size() + netCount) >= PublicWay.num) {
						...
						if (isChecked) {
							...
							chooseBt.setVisibility(View.GONE);
							okButton.setText(Res.getString("finish")+"(" + (Bimp.tempSelectBitmap.size() + netCount) + "/"+PublicWay.num+")");
						}
						isShowOkBt();
					}
				});
