...
public class ConferenceFragment extends FragmentActivity {
    ...
    public ConferenceFragment(int channelId) {
	...
	public void onCreate(Bundle savedInstanceState, PersistableBundle persistentState) {
    	// TODO Auto-generated method stub
        	...
        	View rootView = LayoutInflater.from(this).inflate(R.layout.conference_main, null);
    ...
    private void initView2() {
        if (Constant.conferenceCategoryResp == null || Constant.conferenceCategoryResp.getList() == null)
            return;
        List<CommonEntity> pList = (List<CommonEntity>) Constant.conferenceCategoryResp.getList();
        ...
        for (int i = 0; i < pList.size(); i++) {
            LinearLayout lineDep = (LinearLayout) LayoutInflater.from(this).inflate(R.layout.department, null);
            ...
            if (childList != null) {
                DepartmentAdapter depAdapter = new DepartmentAdapter((List<CommonEntity>) childList, this,
                        nameList);
    ...
    class ABCComparator implements Comparator<CommonEntity> {
        ...
        public int compare(CommonEntity c1, CommonEntity c2) {
            return String.valueOf(c1.getStr1()).compareTo(String.valueOf(c2.getStr1()));
        }
    }

    private void setPager(View rootView) {
        contentPager = (ViewPager) rootView.findViewById(R.id.content_pager_detail);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            categoryList = dbUtils.findAll(Selector.from(ConferenceCategory.class).orderBy("update_date", false));
    ...
    private void getConferenceList(final String id) {
        ...
        XThread thread = new XThread(0, params, Constant.CONFERENCE_LIST_URL, reportNewsHandler);
        thread.start();
    }
    ...
    private void getDepartment() {
        ...
        params.addHeader("Cookie", Constant.sessionId);
        params.addBodyParameter("token", Constant.token);
        new XThread(0, params, Constant.CONFERENCE_CATEGORY_URL, departmentHandler).start();
    ...
    private OnClickListener btnClick = new OnClickListener() {
        ...
        public void onClick(View v) {
            // TODO Auto-generated method stub
            if (v == ibSearch) {
                Intent i = new Intent(ConferenceFragment.this, DbsearchActivity.class);
    ...
    public void btnNewsSelectClick() {
        if (isNewSelectBtnDown) {
            ...
            TranslateAnimation translateAnimation = AnimationFactory.getTranslateAnimation(0, 0, -view1.getHeight(), 0,
                    500);
            ...
            TranslateAnimation translateAnimation = AnimationFactory.getTranslateAnimation(0, 0, 0, -view2.getHeight(),
                    500);
            view2.startAnimation(translateAnimation);
    ...
    private void addCategoryTitle() {
        try {
            ...
            for (Map.Entry<String, String> entry : newsKey.entrySet()) {
                ...
                ConferenceCategory ci = dbUtils.findFirst(Selector.from(ConferenceCategory.class).where(
                        "conference_id", "=", entry.getKey()));
                if (ci == null) {
                    ci = new ConferenceCategory(entry.getKey(), entry.getValue(), 1);
                } else {
                    ci.setStatus(1);
                ...
                adapter.fList.add(new ConferenceListFragment(3, entry.getKey()));
                nameList.add(entry.getKey());
            }
            for (Map.Entry<String, String> entry : Constant.conferenceDelKey.entrySet()) {
    ...
    private class CPagerAdapter extends FragmentStatePagerAdapter {
		...
		private void initData() {
            ...
            if (categoryList == null || categoryList.size() < 4) {
                ...
                ConferenceCategory cc4 = new ConferenceCategory("030805", "传染", 1);
                try {
                ...
                for (int i = 0; i < categoryList.size(); i++) {
                    ...
                    fList.add(new ConferenceListFragment(3, categoryList.get(i).getConferenceId()));
