...
/**
 * @(#)DownAPKService.java Copyright (c) 2013-2014 Simon  All rights reserved.
 */
...
public class DownAPKService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        DownFile(APK_url, APK_dir + File.separator + "WOM.apk");
    ...
    private void initAPKDir() {
        ...
        if (isHasSdcard())// 判断是否插入SD卡
            APK_dir = getApplicationContext().getFilesDir().getAbsolutePath()
                    + "/wiwide/wom/download/";// 保存到app的包名路径下
        else
            APK_dir = Environment.getExternalStorageDirectory()
                    .getAbsolutePath() + "/wiwide/wom/download/";// 保存到SD卡路径下
        File destDir = new File(APK_dir);
        System.out.println("apk_dir --> " + APK_dir);
    ...
    private void DownFile(String file_url, String target_name) {
        mDownLoadHelper = new HttpUtils().download(file_url, target_name, true,
                true, new RequestCallBack<File>() {
                    ...
                    public void onStart() {
                        ...
                        builder.setSmallIcon(R.drawable.wom);
                    ...
                    public void onSuccess(ResponseInfo<File> responseInfo) {
                        ...
                        Uri uri = Uri.fromFile(new File(responseInfo.result
                                .getPath()));
                        installIntent.setDataAndType(uri,
                                "application/vnd.android.package-archive");
                        installIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        PendingIntent mPendingIntent = PendingIntent
                                .getActivity(DownAPKService.this, 0,
                                        installIntent, 0);
...
// startService(intent);
// Toast.makeText(MainActivity.this, "正在后台进行下载，稍后会自动安装",
// Toast.LENGTH_SHORT).show();