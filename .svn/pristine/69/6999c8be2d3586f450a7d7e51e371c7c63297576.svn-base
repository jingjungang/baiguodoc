...
//		Log.d(TAG, "measureViews menuView widthMeasureSpec: " + (menuView.getLayoutParams().width + menuView.getLeft()
//				+ menuView.getRight()));
		View contentView = getChildAt(0);
		contentView.measure(widthMeasureSpec, heightMeasureSpec);
//		Log.d(TAG, "measureViews contentView widthMeasureSpec: " + widthMeasureSpec);
	}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		menuView.layout(contentView.getMeasuredWidth(), 0, contentView.getMeasuredWidth()+width, menuView.getMeasuredHeight());
//		Log.d(TAG, "onLayout menuView: " + (contentView.getMeasuredWidth()+width));
		contentView.layout(0, 0, contentView.getMeasuredWidth(),
				contentView.getMeasuredHeight());
//		Log.d(TAG, "onLayout contentView: " + contentView.getMeasuredWidth());
	}
		...
		case MotionEvent.ACTION_UP:
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				if (velocityX > SNAP_VELOCITY && mCurrentScreen == SCREEN_MAIN) {
					...
					Log.d(TAG, "snapToScreen SCREEN_MENU------- ");
					snapToScreen(SCREEN_MENU);
				} else if (velocityX < -SNAP_VELOCITY
						&& mCurrentScreen == SCREEN_MENU) {
					// Fling hard enough to move right
					Log.d(TAG, "snapToScreen SCREEN_MAIN------- ");
	...
	protected void snapToScreen(int whichScreen) {
		...
		final int newX = (whichScreen - 1) * getChildAt(1).getWidth();
