 ...
 *
 */
public class ReadPicDetailActivity extends SwipeBackActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		actionType = getIntent().getIntExtra("actionType", ACTION_SHOW_MEDCHART);
...
//		adapter.update();
	...
	private void showMedChart(){
		if(Constant.readpicResp!=null && Constant.readpicResp.getObj()!=null){
			ReadPic rp = (ReadPic) Constant.readpicResp.getObj();
			etTitle.setText(rp.getTitle());
			etDesc.setText(rp.getPresentation());
			
			if(rp.getImgs()!=null && !rp.getImgs().isEmpty()){
				list = rp.getImgs();
			}else{
				list = new ArrayList<ImgInfo>();
			}
...
//		XThread thread = new XThread(this, 0, params, Constant.EDIT_MEDCHART_URL, addMedHandler);
//		thread.setShowDia(true);
//		thread.start();
		ConfirmThread thread = new ConfirmThread(Constant.EDIT_READ_PIC_URL, params);
		thread.start();
	}
...
//    	startActivityForResult(Intent.createChooser(intent, "选择图片"), CONSULT_DOC_PICTURE);  
		Intent intent = new Intent(ReadPicDetailActivity.this,
				AlbumActivity.class);
...
////            	filterData(FILTER_TYPE_SCREENING, etfilter.getText().toString());
//            }
//        })
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			try{
				if(list.get(position).isNetImg()){
					...
					holder.image.setImageBitmap(showLitBitmap(list.get(position).getImgPath()));
				}
			}catch(Exception e){}
			...
			convertView.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					if(list.get(position).isNetImg()){
						...
						int index = 1;
						for(int i = 0; i < Bimp.tempSelectBitmap.size(); i++){
							System.out.println("imgpath: " + list.get(position).getImgPath() + " \n" + Bimp.tempSelectBitmap.get(i));
							if(list.get(position).getImgPath().equals(Bimp.tempSelectBitmap.get(i).getImagePath())){
								index = i;
								break;
							}
						}
